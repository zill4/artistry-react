...
const cascade_diff_1 = require("cascade-diff");
var UnitValid;
(function (UnitValid) {
...
class Mask {
    ...
    formatClean(clean, allowLessValid = false) {
        ...
        let diff = cascade_diff_1.default.compare(cleanMask.split('').reverse().join(''), clean.split('').reverse().join(''), compareChars);
    ...
    updateValue(value, oldValue) {
        ...
        let diff = cascade_diff_1.default.compare(oldValue, value);
        diff.reverse();
        let position = 0;
...
function createLCS(diff) {
    ...
    for (var index = 0, length = diff.length; index < length; index++) {
        ...
        switch (diffItem.operation) {
            case cascade_diff_1.DiffOperation.ADD:
                break;
            case cascade_diff_1.DiffOperation.NONE:
                lcs.push(diffItem.item);
                break;
            case cascade_diff_1.DiffOperation.REMOVE:
                break;
        }
    }
    return lcs.join('');
